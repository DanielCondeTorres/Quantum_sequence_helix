...
class QuantumProteinDesign:
    ...
    def _get_probabilities(self, circuit, sampler, backend, force_high_shots=False):
        ...
        if probs is None:
            mc = circuit.copy()
            # Asegurar mediciones si no existen
            if mc.num_clbits == 0:
                mc.measure_all()
            
            # BOOST DE SHOTS: 50k para la foto final
            final_shots = max(self.shots, 50000) if force_high_shots else self.shots
    ...
    def _smart_select(self, probs):
        ...
        indices = np.argsort(probs)[-200:] # Miramos top 200 para no perder nada
        for idx in indices:
    ...
    def solve_qaoa_qiskit(self, p_layers=4, max_iter=800):
        # p=4: Profundidad media (mejor balance que 3 u 8 para este problema)
        ...
        backend = AerSimulator(method='statevector' if self.use_statevector else 'automatic')
        
        # Estrategia de Inicialización Híbrida
        # Mitad Rampa Lineal (bueno para estructura), Mitad Aleatorio (bueno para salir de mínimos locales)
        ...
        n_restarts = 40 # 40 intentos totales
        print(f"   🚀 Ejecutando {n_restarts} rondas (20 Ramp + 20 Random)...")
        
        for i in range(n_restarts):
            # Alternar estrategias
            if i < 20:
                # Estrategia 1: Rampa con ruido
                noise = np.random.uniform(-0.1, 0.1, 2 * p_layers)
                initial_point = ramp_point + noise
            else:
                # Estrategia 2: Puramente Aleatoria (Exploración global)
                initial_point = np.random.uniform(0, 2*np.pi, 2 * p_layers)
            
            optimizer = COBYLA(maxiter=max_iter, tol=0.0001)
            
            # alpha=0.15: CVaR moderado. Demasiado bajo (0.01) es inestable con p=4
            qaoa = CustomQAOA(
                estimator=AerEstimator(), optimizer=optimizer, reps=p_layers,
                initial_point=initial_point,
                backend=backend, shots=self.shots if not self.use_statevector else None, 
                alpha=0.15 
            )
        ...
        try:
            if best_result:
                # Usamos el constructor blindado que hicimos antes
    ...
    def solve_vqe_qiskit(self, layers=2, max_iter=1000):
        print(f"🔥 Solving with Qiskit VQE (layers={layers}, pop=50)...")
        ...
        vqe = CustomVQE(estimator=AerEstimator(), ansatz=ansatz, optimizer=optimizer,
            backend=backend, shots=self.shots if not self.use_statevector else None, alpha=1.0)
